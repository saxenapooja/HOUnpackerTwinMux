...
class HOTwinMuxDigi {
 public:
  ...
  HOTwinMuxDigi(int ieta, int iphi, int bx, int mip, int validbit, int wheel, int sector, int index, int link);
  // HOTwinMuxDigi(int ieta, int iphi, int mip, bool validbit, signed int BX); //better official alternative
  ...
  /// get the valid bit
  int validbit() const {return (theTP_HO>>15)&0x1; }
  ...
  /// get the index
  int index() const {return (theTP_HO>>23)&0x1F; }

  /// get the link value
  int link() const {return (theTP_HO>>28)&0x3; }
